...
import { Paper, Grid, Divider, ButtonGroup, IconButton, Button, Icon, LinearProgress, Popper } from '@material-ui/core';
...
//hook
import { useDigitalEngagementHook } from './useDigitalEngagementHook';
...
export default function Home(props) {
    ...
    const { selectedDigitalEngagementCompany } = state
        ...
        return data.map((d, index) => {
            return (
                <div key={index} style={{ position: 'relative', margin: "10px 0px" }}>
                    ...
                    {/* <div className={classes.center}>
                        <Text value="( Ad Exp " color="gray" />
                        <Text color="gray" float="right"
                            value={d.label === "Basic" ? "< $1k )" : d.label === "Intermediate" ? "$1k - $5k )" : "$5k - $10k )"} />
                    </div> */}
                </div>
            )
        });
        ...
        <div className={classes.root}>
            <Grid container>
                <Grid item xs={12} sm={4} md={4}>
                    {
                        selectedDigitalEngagementCompany.count > 0 ? <CompanyLists /> :
