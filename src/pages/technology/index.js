...
import GroupButton from 'pages/home/components/GroupButton';
...
export default function Home(props) {
    ...
    const { selectedTechnologyCompany } = state
    ...
    useEffect(() => {
        // console.log(company.count, totalDigitalEngagement.count)
    ...
    const renderListItems = (data) => {
        return data.map((d, index) => {
            return (
                <div key={index} style={{ position: 'relative', margin: "26px 0px" }}>
                    <div className={classes.center}>
                        <i className="fa fa-question-circle" style={{ color: "gray", margin: "0px 5px 0px 0px" }} onClick={(e) => handleClick(e, index)} />
        ...
        <div className={classes.root}>
            <Grid container>
                <Grid item xs={12} sm={4} md={4}>
                    {
                        selectedTechnologyCompany.count > 0 ? <CompanyLists /> :
                            <div className={classes.marginBoxShadow} style={{border: "1px solid lightgrey"}}>
                                <Text style={{ fontWeight: 600,borderBottom: "1px solid lightgrey", padding: 10}} value="Technology Introduction"/>
                                <i style={{padding: 10, fontSize:40}} class="fa fa-info-circle" aria-hidden="true"></i>
                                <Text style={{fontSize: 14, fontWeight: 600, padding: 20}} value="Technology view provides a consolidated view of web technologies available on company website. It can be filtered by category and web technology provider."/>
                                <Text style={{fontSize: 14, fontWeight: 600,padding: 20}} value="To view list of companies using say tag management, just click on the specific red bar."/>
